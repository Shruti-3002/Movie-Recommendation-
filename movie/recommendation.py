...
'''Beautiful Soup is a Python package for parsing HTML and XML documents. It creates a parse tree for
 parsed pages that can be used to extract data from HTML, which is useful for web scraping.'''

import re

'''Regular expression or RegEx in Python is denoted as RE (REs, regexes or regex pattern) are imported 
through re module. Python supports regular expression through libraries. RegEx in Python supports various
 things like Modifiers, Identifiers, and White space characters.'''

import requests as HTTP

'''The requests module allows you to send HTTP requests using Python. The HTTP request returns
 a Response Object with all the response data (content, encoding, status, etc).'''
...
fl=PhotoImage(file="marvel.png")
background_label=Label(root,image=fl,bg="skyblue")
...
results = Label(lower_frame, bg=bg_color,fg="blue",anchor='center', justify='center', bd=4,)
